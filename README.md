...
Repository local gồm frontend Next.js và backend Django/Celery. Backend đang sử dụng chính thức là `trungtamtrienlam-BE`, giữ cấu trúc chuẩn đã lấy từ production và chạy bằng Docker Compose.
...
```text
trungtamtrienlam/
├── trungtamtrienlam-BE/        # Backend hiện tại
...
- Backend có file `trungtamtrienlam-BE/.env.local` hợp lệ.
...
```powershell
Set-Location .\trungtamtrienlam-BE
...
Script restore chọn file `.dump` mới nhất trong thư mục `backup/`. Tham số `-ConfirmDestructive` cho phép drop và tạo lại database của riêng stack backend hiện tại trước khi nạp dump; hãy tạo backup mới nếu database hiện tại có dữ liệu quan trọng.

Database dump không chứa media. Media đang sử dụng nằm trong `trungtamtrienlam-BE/app/media` và phải được backup riêng. Xem hướng dẫn đầy đủ tại [Backup dữ liệu từ database](./Backup_dữ_liệu_từ_database.md).
...
Chạy các lệnh sau trong `trungtamtrienlam-BE`:
...
Không tạo các container PostgreSQL/Redis rời bên ngoài Compose của backend hiện tại.

## Tài liệu chi tiết

- [Backend](./trungtamtrienlam-BE/README.md)
