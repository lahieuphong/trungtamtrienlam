...
def align_role_levels_low_to_high(apps, schema_editor):
    Role = apps.get_model('authentication', 'Role')

    Role.objects.filter(is_admin=True, is_deleted=False).update(level=4)
    Role.objects.filter(is_director=True, is_admin=False, is_deleted=False).update(level=3)
...
def reverse_role_levels_to_previous(apps, schema_editor):
    Role = apps.get_model('authentication', 'Role')
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('authentication', '0006_align_185_roles_departments'),
    ]

    operations = [
        migrations.RunPython(align_role_levels_low_to_high, reverse_role_levels_to_previous),
    ]
